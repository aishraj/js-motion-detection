(function() {
	...
	/* Note that Mozilla Firefox 18.0 seems to be buggy with regard to this.
		Need to read up info on MDN. For now works as intended on Chrome 25 */
	...
	function drawVideo() {
		canvasSource.getContext('2d').drawImage(video, 0, 0, video.width, video.height);
	}
	...
	function checkAreas() {
		...
		for (var r=0; r<8; ++r) {
			...
			if (average > 10) {
				...
				playSound(notes[r]);
				canvasSource.className = '';
				canvasSource.classList.add(filters[r]);
